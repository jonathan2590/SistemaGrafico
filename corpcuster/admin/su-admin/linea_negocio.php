...
<div id="myModalA" title="Editar Línea de Negocios" style="overflow-x: hidden;" style="display:none;">
<form action="edito-linea_negocio.php" method="post" enctype="multipart/form-data" name="form_editar">
...
<div id="myModalB" title="Agregar Línea de Negocios" style="overflow-x: hidden;" style="display:none;">
<form action="agrego-linea_negocio.php" method="post" enctype="multipart/form-data" name="form_agregar">
<table>
    <tr>
    ...
    <tr>     
      <th >
        <br/>
            <label >Departamento: </label>
                    <select name="select_departamento_a" id="select_departamento_a" class="form-control" >
                    </select>            
      </th>               
    </tr>              
...
var nombre=jQuery("#select_departamento").val();
...
if (nombre <= '0')
  {
    ...
    return false;
     ...
     BootstrapDialog.show({
                                title: 'Actualización en Línea de Negocios',
                                message: '¿Seguro desea editar el registro ?',
                                buttons: [{
                                    id: 'btn-ok',   
                                    ...
                                    $.ajax({     data: {"usuario" : jQuery("#codigo").val()  ,tipos: jQuery("#select_departamento").val(),codigotipos: jQuery("#codigo_e").val(),select_estado:jQuery("#select_estado_a").val() },
                                                 type: "POST",
                                                  dataType: "json",
                                                  url: "edito-linea_negocio.php",
                                                    })
                                                 .done(function( response ) {
                                                  if( response.success ) 
                                                  {
                                                       if ( console && console.log ) 
                                                       {
                                                           ...
                                                           mensaje('Se modifico Correctamente la Línea de Negocios' );
                                                           dialog.dialog("close");
                                                           listar_tipos();//location.reload(true)
                                                      }
                                                   } 
                                                   else 
                                                   {
...
var nombre=jQuery("#select_departamento_a").val();
...
if (nombre <= '0')
  {
    ...
    return false;
     ...
     BootstrapDialog.show({
                                title: 'Agregar la Línea de Negocios',
                                message: '¿Seguro desea agregar el registro <b>'+jQuery("#codigo_a").val()+'</b> ?',
                                buttons: [{
                                    id: 'btn-ok',   
                                    icon: 'glyphicon glyphicon-check',       
                                    label: 'OK',
                                    ...
                                    autospin: true,
                                    action: function(dialogRef){    
                                     $.ajax({     data: {codigo_a: jQuery("#codigo_a").val(),nombre_a: jQuery("#select_departamento_a").val() },
                                                 type: "POST",
                                                  dataType: "json",
                                                  url: "agrego-linea_negocio.php",
                                                    })
                                                 .done(function( response ) {
                                                  if( response.success ) 
                                                  {
                                                       if ( console && console.log ) 
                                                       {
                                                           ...
                                                           mensaje('Se Agrego Correctamente la Línea de Negocios: ' + jQuery("#codigo_a").val());
...
$.ajax({
                     type: "POST",
                      dataType: "json",
                      url: "../class/consulta_linea_negocio.php",
                        })
                     .done(function( response ) {
                                ...
                                jQuery.each( response.data, function( key, val ) 
                                  {
                                      ...
                                      addDataTable((val.id),(val.id_depa),(val.nombre),val.estado,estado);
                                  });
...
//se inicializa el documento
 jQuery(document).ready(function() {
  ...
  jQuery("#codigo_a").change(function()
    {
var mensaje="";
mensaje="El código en Línea de Negocios " + this.value + " ya existe";

$.ajax({
                     
                     type: "POST",
                      dataType: "json",
                      data: {codigotipo : this.value},  
                      url: "../class/consulta_linea_negocio.php",
                        })
                     .done(function( response ) {
...
$.getJSON( "../class/consulta_departamento.php",{format: "json"}) 
        .done(function( data ) {
         
         if(data.success==true)
         {
         
          var miselect=$("#select_departamento");
           var miselect_a=$("#select_departamento_a");
          ...
          miselect.append('<option  value="'+(val.depart_id)+'">'+val.nombre_depart+'</option>'); 
          miselect_a.append('<option  value="'+(val.depart_id)+'">'+val.nombre_depart+'</option>'); 
...
/*jQuery('#st_usuarios tbody').on( 'click', 'tr', function () {
 ...
 jQuery('#st_usuarios tbody').on( 'click', 'button', function () {
        ...
        {        
         BootstrapDialog.show({
                                title: 'Eliminación de Línea de Negocios',
                                                 ...
                                                 .done(function( response ) {
                                                  if( response.success ) 
                                                  {
                                                       if ( console && console.log ) 
                                                       {
                                                           ...
                                                           mensaje('Se Elimino Correctamente la Línea de Negocios: ' + data[1]);
                                                           listar_tipos();//location.reload(true)
                                                      }
                                                   } 
                                                   else 
                ...
                else
                {
                  ...
                  jQuery("#select_departamento").val(data[1])                  
                  jQuery("#select_estado_a").val(data[3])
                }
                    } );
